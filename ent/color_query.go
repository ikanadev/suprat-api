...
func (cq *ColorQuery) QueryActivities() *ActivityQuery {
...
func (cq *ColorQuery) QueryMeasurements() *MeasurementQuery {
	query := &MeasurementQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// First returns the first Color entity in the query. Returns *NotFoundError when no color was found.
...
// FirstID returns the first Color id in the query. Returns *NotFoundError when no id was found.
func (cq *ColorQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (cq *ColorQuery) FirstIDX(ctx context.Context) int {
...
// Only returns the only Color entity in the query, returns an error if not exactly one entity was returned.
...
// OnlyID returns the only Color id in the query, returns an error if not exactly one id was returned.
func (cq *ColorQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (cq *ColorQuery) OnlyIDX(ctx context.Context) int {
...
func (cq *ColorQuery) IDs(ctx context.Context) ([]int, error) {
	var ids []int
...
func (cq *ColorQuery) IDsX(ctx context.Context) []int {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *ColorQuery) Clone() *ColorQuery {
	...
	return &ColorQuery{
		...
		withActivities:   cq.withActivities.Clone(),
		withMeasurements: cq.withMeasurements.Clone(),
...
func (cq *ColorQuery) WithActivities(opts ...func(*ActivityQuery)) *ColorQuery {
	query := &ActivityQuery{config: cq.config}
...
func (cq *ColorQuery) WithMeasurements(opts ...func(*MeasurementQuery)) *ColorQuery {
	query := &MeasurementQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withMeasurements = query
	return cq
}
...
//		Scan(ctx, &v)
//
func (cq *ColorQuery) Select(field string, fields ...string) *ColorSelect {
	selector := &ColorSelect{config: cq.config}
...
func (cq *ColorQuery) sqlAll(ctx context.Context) ([]*Color, error) {
	...
	if query := cq.withActivities; query != nil {
		...
		query.Where(predicate.Activity(func(s *sql.Selector) {
			s.Where(sql.InValues(color.ActivitiesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.color_activities
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "color_activities" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "color_activities" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Activities = append(node.Edges.Activities, n)
		}
	}

	if query := cq.withMeasurements; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Color)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Measurements = []*Measurement{}
		}
		query.withFKs = true
		query.Where(predicate.Measurement(func(s *sql.Selector) {
			s.Where(sql.InValues(color.MeasurementsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.color_measurements
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "color_measurements" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "color_measurements" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Measurements = append(node.Edges.Measurements, n)
		}
	}
...
func (cq *ColorQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if ps := cq.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector, color.ValidColumn)
			}
		}
	}
	return _spec
...
func (cq *ColorQuery) sqlQuery() *sql.Selector {
	builder := sql.Dialect(cq.driver.Dialect())
	t1 := builder.Table(color.Table)
	selector := builder.Select(t1.Columns(color.Columns...)...).From(t1)
	if cq.sql != nil {
		selector = cq.sql
		selector.Select(selector.Columns(color.Columns...)...)
	}
	for _, p := range cq.predicates {
		p(selector)
	}
	for _, p := range cq.order {
		p(selector, color.ValidColumn)
	}
	if offset := cq.offset; offset != nil {
...
// ColorGroupBy is the builder for group-by Color entities.
...
func (cgb *ColorGroupBy) sqlQuery() *sql.Selector {
	...
	for _, fn := range cgb.fns {
		columns = append(columns, fn(selector, color.ValidColumn))
	}
	return selector.Select(columns...).GroupBy(cgb.fields...)
}

// ColorSelect is the builder for select fields of Color entities.
...
func (cs *ColorSelect) sqlQuery() sql.Querier {
	selector := cs.sql
