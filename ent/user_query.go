...
func (uq *UserQuery) QueryMeasurements() *MeasurementQuery {
	query := &MeasurementQuery{config: uq.config}
...
func (uq *UserQuery) QueryActivities() *ActivityQuery {
	query := &ActivityQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withMeasurements: uq.withMeasurements.Clone(),
...
func (uq *UserQuery) WithMeasurements(opts ...func(*MeasurementQuery)) *UserQuery {
	query := &MeasurementQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withMeasurements = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withMeasurements; query != nil {
		...
		query.Where(predicate.Measurement(func(s *sql.Selector) {
			s.Where(sql.InValues(user.MeasurementsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_measurements
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_measurements" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_measurements" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := uq.withActivities; query != nil {
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_activities" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_activities" returned %v for node %v`, *fk, n.ID)
			}
